...
impl Content {
    ...
    fn len(&self) -> usize {
        let cur_chunk_count = self.data.len();
        if cur_chunk_count > 0 {
            CHUNK_LEN * (cur_chunk_count - 1)
                + self.data[cur_chunk_count - 1].as_ref().unwrap().len()
        } else {
            0
        }
    }
    ...
    fn resize_zero(&mut self, new_len: usize) {
        let cur_chunk_count = self.data.len();
        let new_chunk_count = (new_len + CHUNK_LEN - 1) / CHUNK_LEN;
        let new_chunk_len = {
        ...
        if new_chunk_count > cur_chunk_count {
            // Expand the chunks
            self.data.resize(new_chunk_count, None);
            // The chunks between cur_chunk_idx(cur_chunk_count - 1) and
            // new_chunk_idx(new_chunk_count - 1) are hole, we do not
            ...
            if cur_chunk_count > 0 {
                ...
                self.data[cur_chunk_count - 1]
                    .as_mut()
                    .unwrap()
                    .resize(CHUNK_LEN, 0);
            }
            // Initialize the new tail chunk
            self.data[new_chunk_count - 1] = Some(Self::new_empty_chunk_with_len(new_chunk_len));
        } else {
            // Truncate the number of chunks
            self.data.truncate(new_chunk_count);
            if new_chunk_count > 0 {
                if self.data[new_chunk_count - 1].is_none() {
                    // Chunk is a hole, initialize it
                    self.data[new_chunk_count - 1] =
                        Some(Self::new_empty_chunk_with_len(new_chunk_len));
                } else {
                    // Resize new tail chunk
                    self.data[new_chunk_count - 1]
                        .as_mut()
                        .unwrap()
                        .resize(new_chunk_len, 0);
                }
            }
        }
    }
    ...
    fn copy_from_slice(&mut self, offset: usize, src: &[u8]) {
        ...
        let rest_len_in_first_chunk = CHUNK_LEN - chunk_offset;
        let first_chunk_len = rest_len_in_first_chunk.min(src.len());
        if self.data[chunk_idx].is_none() {
            ...
            let target = &mut self.data[chunk_idx].as_mut().unwrap()
                [chunk_offset..chunk_offset + first_chunk_len];
            let buf = &src[..first_chunk_len];
            target.copy_from_slice(buf)
        }
        if rest_len_in_first_chunk < src.len() {
            let mut rest_len = src.len() - rest_len_in_first_chunk;
            let mut chunk_idx = chunk_idx + 1;
            let mut offset = rest_len_in_first_chunk;
            while rest_len > 0 {
                let copy_len = rest_len.min(CHUNK_LEN);
                if self.data[chunk_idx].is_none() {
                    // Chunk is a hole, initialize it from slice
                    self.data[chunk_idx] = Some(Self::new_chunk_from_slice_at_offset(
    ...
    fn copy_to_slice(&self, offset: usize, target: &mut [u8]) -> usize {
        ...
        let rest_len_in_first_chunk = CHUNK_LEN - chunk_offset;
        let first_chunk_len = rest_len_in_first_chunk.min(len);
        if self.data[chunk_idx].is_none() {
            // Chunk is a hole, set null bytes
            for item in target.iter_mut().take(first_chunk_len) {
                *item = 0;
            }
        } else {
            target[..first_chunk_len].copy_from_slice(
                &self.data[chunk_idx].as_ref().unwrap()
                    [chunk_offset..chunk_offset + first_chunk_len],
            );
        }

        if rest_len_in_first_chunk < len {
            let mut rest_len = len - rest_len_in_first_chunk;
            let mut chunk_idx = chunk_idx + 1;
            let mut offset = rest_len_in_first_chunk;
            while rest_len > 0 {
                let copy_len = rest_len.min(CHUNK_LEN);
